...
As seen above, this file contains text-based ranks that won't work in a GLM. NA's get converted to zeroes. 999 is equivalent to NA in the Alberta data. Both indicate 'unranked.
...
``` {r}
totalBirds <- merge(totalBirds, mastermatrix[c("ForestID", "subregion.name", "standtype.name", "ageclass", "tpr.name", "FinalRank")], by="ForestID", all.x=T) # merge by ForestID. Keep only sampled ForestIDs (i.e., those with survey data)
totalBirds <- totalBirds[c("ForestID", "subregion.name", "standtype.name", "ageclass", "tpr.name", "SS", "PKEY", "Count", "TOTA", "FinalRank")]
...
``` {r}
totalBirds$standtype.name<-gsub("Mixedwood-conifer", "MW-C", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("Mixedwood-decid", "MW-D", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("Mixedwood-decidPl", "MW-D-Pl", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("Mixedwood-decidSb", "MW-D-Sb", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("Mixedwood-decidSw", "MW-D-Sw", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("Mixedwood-Pl", "MW-Pl", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("Mixedwood-Sb", "MW-Sb", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("Mixedwood-Sw", "MW-Sw", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("PureBlackSpruce", "Sb", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("PureConifer-Allcon", "C-all", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("PureConifer-AllCon", "C-all", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("PureConifer-Pl", "C-Pl", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("PureConifer-Sx", "C-Sx", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("PureHardwood", "D", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("PurePine", "P", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("PureWhiteSpruce", "Sw", totalBirds$standtype.name)
totalBirds$standtype.name<-gsub("Unidentified", "UnID", totalBirds$standtype.name)

totalBirds$subregion.name<-gsub("Alpine-Subalpine","AlpSub", totalBirds$subregion.name)
totalBirds$subregion.name<-gsub("Athabasca Plain ALPAC","AthPla", totalBirds$subregion.name)
totalBirds$subregion.name<-gsub("Boreal Subarctic","BorSub", totalBirds$subregion.name)
totalBirds$subregion.name<-gsub("Central Mixedwood","CenMix", totalBirds$subregion.name)
totalBirds$subregion.name<-gsub("Dry Mixedwood","DryMix", totalBirds$subregion.name)
totalBirds$subregion.name<-gsub("Northern Mixedwood","NorMix", totalBirds$subregion.name)
totalBirds$subregion.name<-gsub("Lower Boreal Highlands","LowBorHig", totalBirds$subregion.name)
totalBirds$subregion.name<-gsub("Lower Foothills","LowFoo", totalBirds$subregion.name)
totalBirds$subregion.name<-gsub("Upper Boreal Highlands","UppBorHig", totalBirds$subregion.name)
totalBirds$subregion.name<-gsub("Upper Foothills","UppFoo", totalBirds$subregion.name)
totalBirds$subregion.name<-gsub("Unidentified","UnID", totalBirds$subregion.name)
totalBirds$subregion.name<-gsub("Montane","Mon", totalBirds$subregion.name)

totalBirds$ageclass<-gsub("a.Age 0-2 Y","0-2y",totalBirds$ageclass)
totalBirds$ageclass<-gsub("b.Age 3-30 Y","a.3-30y",totalBirds$ageclass)
totalBirds$ageclass<-gsub("c.Age 31-60 Y","b.31-60y",totalBirds$ageclass)
totalBirds$ageclass<-gsub("d.Age 61-80 Y","c.61-80y",totalBirds$ageclass)
totalBirds$ageclass<-gsub("e.Age 81-100 Y","d.81-100y",totalBirds$ageclass)
totalBirds$ageclass<-gsub("f.Age 101-140 Y","e.101-140y",totalBirds$ageclass)
totalBirds$ageclass<-gsub("g.Age 141+","f.>140y",totalBirds$ageclass)

totalBirds$tpr.name<-gsub("Unproductive","a.Unproductive",totalBirds$tpr.name)
totalBirds$tpr.name<-gsub("Fair","b.Fair",totalBirds$tpr.name)
totalBirds$tpr.name<-gsub("Medium","c.Medium",totalBirds$tpr.name)
totalBirds$tpr.name<-gsub("Good","d.Good",totalBirds$tpr.name)
...
#levels(mastermatrix$ageclass) <- c("3-30 yrs", "31-80 yrs", "81-120 yrs", "121-250 yrs", ">250 yrs")
#levels(mastermatrix$tpr.name) <- c("<10.5 m", "10.5-19.4 m", "19.5-28.4 m", "> 28.5 m")
...
mastermatrix$standtype.name<-gsub("Mixedwood-conifer", "MW-C", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("Mixedwood-decid", "MW-D", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("Mixedwood-decidPl", "MW-D-Pl", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("Mixedwood-decidSb", "MW-D-Sb", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("Mixedwood-decidSw", "MW-D-Sw", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("Mixedwood-Pl", "MW-Pl", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("Mixedwood-Sb", "MW-Sb", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("Mixedwood-Sw", "MW-Sw", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("PureBlackSpruce", "Sb", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("PureConifer-Allcon", "C-all", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("PureConifer-AllCon", "C-all", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("PureConifer-Pl", "C-Pl", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("PureConifer-Sx", "C-Sx", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("PureHardwood", "D", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("PurePine", "P", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("PureWhiteSpruce", "Sw", mastermatrix$standtype.name)
mastermatrix$standtype.name<-gsub("Unidentified", "UnID", mastermatrix$standtype.name)

mastermatrix$subregion.name<-gsub("Alpine-Subalpine","AlpSub", mastermatrix$subregion.name)
mastermatrix$subregion.name<-gsub("Athabasca Plain ALPAC","AthPla", mastermatrix$subregion.name)
mastermatrix$subregion.name<-gsub("Boreal Subarctic","BorSub", mastermatrix$subregion.name)
mastermatrix$subregion.name<-gsub("Central Mixedwood","CenMix", mastermatrix$subregion.name)
mastermatrix$subregion.name<-gsub("Dry Mixedwood","DryMix", mastermatrix$subregion.name)
mastermatrix$subregion.name<-gsub("Northern Mixedwood","NorMix", mastermatrix$subregion.name)
mastermatrix$subregion.name<-gsub("Lower Boreal Highlands","LowBorHig", mastermatrix$subregion.name)
mastermatrix$subregion.name<-gsub("Lower Foothills","LowFoo", mastermatrix$subregion.name)
mastermatrix$subregion.name<-gsub("Upper Boreal Highlands","UppBorHig", mastermatrix$subregion.name)
mastermatrix$subregion.name<-gsub("Upper Foothills","UppFoo", mastermatrix$subregion.name)
mastermatrix$subregion.name<-gsub("Unidentified","UnID", mastermatrix$subregion.name)
mastermatrix$subregion.name<-gsub("Montane","Mon", mastermatrix$subregion.name)

mastermatrix$ageclass<-gsub("a.Age 0-2 Y","0-2y",mastermatrix$ageclass)
mastermatrix$ageclass<-gsub("b.Age 3-30 Y","a.3-30y",mastermatrix$ageclass)
mastermatrix$ageclass<-gsub("c.Age 31-60 Y","b.31-60y",mastermatrix$ageclass)
mastermatrix$ageclass<-gsub("d.Age 61-80 Y","c.61-80y",mastermatrix$ageclass)
mastermatrix$ageclass<-gsub("e.Age 81-100 Y","d.81-100y",mastermatrix$ageclass)
mastermatrix$ageclass<-gsub("f.Age 101-140 Y","e.101-140y",mastermatrix$ageclass)
mastermatrix$ageclass<-gsub("g.Age 141+","f.>140y",mastermatrix$ageclass)

mastermatrix$tpr.name<-gsub("Unproductive","a.Unproductive",mastermatrix$tpr.name)
mastermatrix$tpr.name<-gsub("Fair","b.Fair",mastermatrix$tpr.name)
mastermatrix$tpr.name<-gsub("Medium","c.Medium",mastermatrix$tpr.name)
mastermatrix$tpr.name<-gsub("Good","d.Good",mastermatrix$tpr.name)
