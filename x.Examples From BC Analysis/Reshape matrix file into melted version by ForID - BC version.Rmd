...
``` {r, eval=T}
## RANK
cofi <- read.csv("fromNB.csv")#file path name too long for my computer so I shortened "area_summary_with_matrix-RANK-zSpGroup.csv" to "fromNB"
...
CREATE FOREST ID
``` {r, eval=T}
ranks1$ForID <- with(ranks1, paste(BEC, code_use, ForAge, ForHt, sep="."))
ranks <- ranks1[c("ForID", "Rank")]#Changed "SpeciesGroup" to "code_use"
``` 
