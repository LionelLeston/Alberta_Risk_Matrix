...
load(paste0(path,"RDataFiles/mastermatrix.RData"))
...
``` {r}
ranks <- read.csv(paste0(path,"rawData/ForID_Ranks_final_long.csv"), head=T)
...
load(paste0(path,"RDataFiles/TOTAlcount.per.PKEY.withForID.RData"))
...
``` {r}
load(paste0(path,"RDataFiles/TOTA_offsets-v3_2016-12-01.RData"))
qs.off <- OFF
...
``` {r}
load(paste0(path,"RDataFiles/summarizedRanks.RData"))
```
...
``` {r}
save(totalBirds, file=paste0(path,"RDataFiles/totalBirds_clean.RData"))
```
...
``` {r}
if ('fit.dens.forid_tot.RData' %in% dir(paste0(path,"RDataFiles"))) {
  load(paste0(path,"RDataFiles/fit.dens.forid_tot.RData")) # loads the saved results if it's already run and saved
}
...
if (exists("fit.dens.forid_tot")==FALSE) { #conditional statement; won't run if the file was already loaded from RData
  ...
  save(fit.dens.forid_tot, file=paste0(path,"RDataFiles/fit.dens.forid_tot.RData")) # save it so i don't need to run again. 
}
...
save(dens_per_ForID, file=paste0(path,"RDataFiles/totalBirds_withpreds.RData"))
```
