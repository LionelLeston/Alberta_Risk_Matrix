...
We resolved this issue in November 2018 with Jeremy Beal at FORSITE.
The mbca.avi contains just the AVI-defined forest polygons. In the FORSITE risk matrix AVI, more recent fires and cutblocks were added, splitting the polygons into more.
...
ageclass.levels <- factor(unique(ariskmatrixAVI.A$ageclass)[-which(unique(ariskmatrixAVI.A$ageclass)=="NA")&-which(unique(ariskmatrixAVI.A$ageclass)=="Age 0-2 Y")])
