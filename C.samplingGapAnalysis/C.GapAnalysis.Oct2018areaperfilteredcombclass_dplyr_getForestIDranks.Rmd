---
title: "C.GapAnalysis.CreateAlbertaMasterMatrixAndRanksMatrix"
...
Each data frame above contains the same number of unique observations as rows, indicating no duplicate observations, for example `r unique(riskmatrixAVI1)` unique observations and `r nrow(riskmatrixAVI1)` rows in riskmatrixAVI1.
...
```{r summarize-area-and-create-ranks-list}
library(dplyr)
...
```{r summarize-area-and-create-gap-analysis}
