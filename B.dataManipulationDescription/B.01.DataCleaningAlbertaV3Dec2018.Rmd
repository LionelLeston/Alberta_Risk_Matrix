...
LOOKUP.Subregion<-read.csv("lookup/LOOKUP.Subregion.csv", header=TRUE)
#add abbreviated subregion names
AlbertaMastermatrixB1<-merge(AlbertaMastermatrix, LOOKUP.Subregion, by=c("subregion.name"), all.x=T)
LOOKUP.StandType<-read.csv("lookup/LOOKUP.StandType.csv", header=TRUE)
#add abbreviated stand type and coarse forest type names
AlbertaMastermatrixB2<-merge(AlbertaMastermatrixB1, LOOKUP.StandType, by=c("standtype.name"), all.x=T)
#add abbreviated age class names
LOOKUP.ForestAge<-read.csv("lookup/LOOKUP.ForestAge.csv", header=TRUE)
AlbertaMastermatrixB3<-merge(AlbertaMastermatrixB2, LOOKUP.ForestAge, by=c("ageclass"), all.x=T)
LOOKUP.TimberProd<-read.csv("lookup/LOOKUP.TimberProd.csv", header=TRUE)
AlbertaMastermatrixB<-merge(AlbertaMastermatrixB3, LOOKUP.TimberProd, by=c("tpr.name"), all.x=T)
#add alternate name for ForestID
AlbertaMastermatrixB$matrix.forest.class<-AlbertaMastermatrixB$ForestID

matrix.qs <- AlbertaMastermatrixB#quicksaves AlbertaMastermatrixB4 up to this point

write.csv(AlbertaMastermatrixB, file="Output/AlbertaMastermatrixB.csv")
save(AlbertaMastermatrixB, file="RDataFiles/AlbertaMastermatrixB.Rdata")
#saved under a new name, so that if errors are made, original master matrix doesn't have to be recreated.
```
...
####3. Load "ranks" file, then reformat ForestID column in "ranks" to match ForestID column in AlbertaMastermatrixB and get rid of unranked stands
...
write.csv(ranks, file="Output/ReducedAlbertaForID_Ranks_final_long.csv")
kable(head(ranks, 10))
kable(t(table(ranks$FinalRank)))
```

The ForestID column in "ranks" previously needed to be formatted to match up with the ForestID column in "AlbertaMastermatrix(B)". For some reason it doesn't now, since October 16, but keep the chunk below just in case the issue comes up when importing the risk matrix AVI or ranks again.
...
Previously in script A, we used the point counts to get the number of points for each type of forest matrix class. Now in script B, we are going to merge the actual point counts and their offsets with the risk matrix AVI data associated with each point count.
...
#now reformat ForestID to match formatting in AlbertaMastermatrixB
