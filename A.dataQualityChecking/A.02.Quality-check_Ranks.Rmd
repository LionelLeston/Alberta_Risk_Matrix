...
### Goal: Compare Ranks extracted from Forsite's AVI-based shapefile and those from the Lisa's rank matrix file
...
avi.validstands <- read.csv("rawData/avi.validStandsOnly.csv", header=T)
```

``` {r check.unique.ranks.forsite, echo=T}
FORSITEmatrixranksbyForID <- aggregate(avi.validstands$Rating, by=list(ForestID=avi.validstands$ForestID), FUN=function(x) {unique(x)})
FORSITEmatrixnumranksbyForID <- aggregate(avi.validstands$Rating, by=list(ForestID=avi.validstands$ForestID), FUN=function(x) {length(unique(x))})
