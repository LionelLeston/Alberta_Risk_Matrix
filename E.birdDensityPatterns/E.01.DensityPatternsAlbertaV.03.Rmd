...
Contains all possible forest stand type combinations, based on factorial combination of all levels of all 4 attributes. The combination of forest attributes is the unique ForestID for a stand type. Data should have already been cleaned by this point, with abbreviated values for stand type, subregion, and age class in newly created columns.
...
Contains stand ranks assigned to ForestID. Note that ranks were previously quality-checked and that ForestID has been previously formatted to match the style used in "AlbertaMastermatrix". These are the final ranks. 
...
load("RDataFiles/AlbertaCounts.RData")#loads data frame "merged2gether"
...
``` {r}
load("RDataFiles/AlbertaTOTAoffsets-v3_2018-03-02.RData")
load("RDataFiles/AlbertaTOTAoffsets-v3data_2018-03-02.RData")
qs.off <- OFF
...
``` {r}
load("RDataFiles/AlbertasummarizedRanks.RData")
```
