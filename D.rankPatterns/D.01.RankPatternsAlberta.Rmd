...
``` {r}
#ranks$FinalRank<-as.factor(ranks$FinalRank)#changed from Rating to FinalRank
#levels(ranks$FinalRank)
ranks$FinalRank[ranks$FinalRank==999]<-"NA"
ranks$FinalRank[ranks$FinalRank==0]<-"NA"
#levels(ranks$FinalRank)
ranks$FinalRank <-as.numeric(ranks$FinalRank) #as.numeric(as.character(ranks$Rating))
...
mastermatrix <- mastermatrix[c("ForestID", "subregion.name", "standtype.name", "ageclass", "tpr.name", "FinalRank", "ForestArea")]#"Rating" removed: not relevant
...
save(rank.aggregates, file=paste0(path,"RDataFiles/AlbertasummarizedRanks.RData"))
```
...
![Distribution of ranks over the Alberta Boreal Forest Region; map generated from risk matrix V.3. geodatabase received from FORSITE](../images/AlbertaRankingsMap.jpg)
...
#either abbreviations need to be used for subregion names or x-axis labels should have vertical text (switched to facet theme 1)
```
...
ggplot(mastermatrix, aes(x=as.numeric(as.character(FinalRank)), fill=subregion.name)) + geom_histogram(bins=6, col="black") + scale_x_continuous(labels=c(1:6), breaks=1:6) + ylab("Number of Stand Types") + scale_fill_manual(values=palsub, name = "Subregion") + myfacettheme3 + xlab("Stand Rank")
``` 
...
ggplot(mastermatrix, aes(x=FinalRank, fill=standtype.name)) + geom_histogram(bins=6, col="black") + myfacettheme3 + scale_fill_manual(values=palstand, name="Dominant Stand Type") + scale_x_continuous(labels=c(1:6), breaks=1:6) + ylab("Number of Stand Types") + guides(fill=guide_legend(ncol=1)) + xlab("Stand Rank")
```
