...
The most recent previous version of this analysis was run in October, and includes the creation of an AlbertaMastermatrix containing observations of cumulative area, subregion, stand, age-class, and timber productivity rating for each ForestID, and a separate list of risk matrix ranks for each ForestID. This version differs in that age classes are based on the ages assigned to polygons in the original risk matrix AVI.
...
It has 6120960 records/polygons. It has more polygons than the original composite AVI (mbca_avi) due to additions of 
the most recent harvest areas and fires that were lacking in the composite AVI.
...
## Question to resolve: why are there twice as many polygons in the matrix AVI than in the basic AVI (RESOLVED)
There are twice as many polygons in the risk matrix AVI as the basic composite AVI due to the addition of more recent harvest blocks and fires setting back age.



## Step 2. Create age classes in R based on age since stand origin

In the risk matrix for Alberta, foresters assign forest stands to age classes that are meaningful for foresters, but are probably meaningful for breeding birds as well. The following chunk is used to calculate the age class of each polygon based on the age that was used when calculating the matrix risk rankings. 

It appears to be an artifact now, because age-class was originally calculated in the AVI attribute table in ArcGIS before reading it into R, but the chunk would be necessary if a new copy of the risk matrix were received.

#keep in script for now
...
#appears to be an artifact from a time when tpr.name wasn't calculated in ArcGIS
#keep in script for now
...
AlbertaForID_Ranks_final_long<-ariskmatrixAVI.A%>%
...
At this point Area.1000 (area in 1000-ha units) and survey effort can be calculated. The file is then stored in the Output folder since it is based on output data.
...
#write to CSV then read in again
pointsbyForestID<-read.csv("Output/pointsbyForestID.csv", header=TRUE)
...
pointsbyForestID<-merge(pointcountsformatrix[,c("ForestID", "subregion.name", "standtype.name", "ageclass", "tpr.name")], pointsbyForestID)
...
areabyForestID<-read.csv("Output/areabyForestID_Nov20.csv", header=TRUE)
#use the most recent version, which shows the difference in area calculated from the AVI before we realized that Forsite personnel who prepared the AVI did not recalculate areas after adding recent cutblocks and burns
AlbertaMastermatrix<-merge(pointsbyForestID, areabyForestID, by=c("ForestID","subregion.name","standtype.name","ageclass","tpr.name"), all=TRUE)
AlbertaMastermatrix$Area.1000units<-AlbertaMastermatrix$AreaNov20/1000
